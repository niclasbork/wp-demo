			...
			<div { ...blockProps }>
				{slides.length > 0 ? (
					...
					>
						<div className="swiper-wrapper">
							{slides.map((slide, index) => (
								<div className="swiper-slide" key={index}>
									<div className="demo-slider-block__slide" style={{ backgroundImage: `url(${slide.mediaUrl})` }}>
										<div className="demo-slider-block__overlay"></div>
										<div className="demo-slider-block__content">
											{slide.heading && <h2 className="demo-slider-block__heading">{slide.heading}</h2>}
											{slide.description && <p className="demo-slider-block__description">{slide.description}</p>}
										</div>
									</div>
								</div>
							))}
						...
						{navigation && <div className="swiper-button-prev"></div>}
						{navigation && <div className="swiper-button-next"></div>}
					</div>
				) : (
